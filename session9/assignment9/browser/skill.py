...
# Writes to logs/browser_actions_<timestamp>.log — handler is attached by
# run.py at startup. Falls back to stderr if nothing configured it (e.g.
# when the skill is exercised directly from a test).
log = logging.getLogger("browser.actions")
...
class BrowserSkill:
    ...
    async def run(self, node: NodeSpec) -> AgentResult:
        ...
        if html:
            ...
            if block:
                log.info("[precondition] gateway_blocked: %s marker on %s", block, final_url)
        ...
        if selectors:
            ...
            if det is not None:
                log.info("[Layer 2a] deterministic succeeded")
                return det
            log.info("[Layer 2a] deterministic failed — escalating")
        ...
        if vis_result.success:
            log.info("[Layer 3] vision succeeded")
            return self._pack_driver("vision", url, goal, vis_result,
                                     final_url=vis_result.final_url,
                                     elapsed=time.time() - t0)

        log.info("[Layer 3] vision exhausted — all layers failed")
    ...
    # ── per-turn logging callback (passed to the driver as `on_step`) ──────
    @staticmethod
    def _log_step(layer: str, rec, snap) -> None:
        if layer == "vision":
            for a in rec.actions:
                if a.get("type") in ("click", "type") and a.get("mark") is not None:
                    el = snap.by_id(int(a["mark"]))
                    coord = (el.cx, el.cy) if el else ("?", "?")
                    log.info("[Layer 3] turn %d: chosen_box=%s, coordinate=(%s,%s)",
                             rec.turn, a["mark"], coord[0], coord[1])
            if not any(a.get("type") in ("click", "type") for a in rec.actions):
                log.info("[Layer 3] turn %d: action=%s", rec.turn,
                         rec.actions[0].get("type") if rec.actions else "(none)")
        else:
            for a in rec.actions:
                log.info("[Layer 2b] turn %d: action=%s", rec.turn, a)
    ...
    async def _drive(self, DriverCls, url, goal, client, artifacts_dir,
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                drv = DriverCls(page, client, cfg, on_step=self._log_step)
