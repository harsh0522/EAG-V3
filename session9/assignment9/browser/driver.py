...
def _is_dropdown_trigger(name: str) -> bool:
    """Name patterns that mark a dropdown trigger: trailing `▾`/`:`, or a
    leading `Sort:` label. Their popup options only render after the click,
    so chaining a second action the same turn would target a stale element."""
    name = (name or "").strip()
    return name.endswith("▾") or name.endswith(":") or name.startswith("Sort:")


def _enforce_turn_rules(actions: list[dict], snap: PageSnapshot) -> list[dict]:
    """Code-level guardrails on top of the prompt's instructions (models
    don't always follow prose):
      - dropdown-as-fence: if the first non-`done` action targets a
        dropdown trigger, it is the ONLY action this turn (its popup
        options don't exist in the DOM yet).
      - hard cap of 2 actions per turn overall.
    """
    capped = actions[:2]
    for i, a in enumerate(capped):
        if a.get("type") == "done":
            return capped[: i + 1]
        mark = a.get("mark")
        el = snap.by_id(int(mark)) if mark is not None else None
        if el and _is_dropdown_trigger(el.name):
            return capped[: i + 1]
    return capped
...
class BaseDriver:
    ...
    def __init__(self, page: Page, client: V9Client, config: DriverConfig,
        ...
        # Optional callback `(layer_name, record, snapshot) -> None` invoked
        # right after each turn is recorded — lets the skill log
        # `[Layer 2b] turn N: action=<action>` / `[Layer 3] turn N: chosen_box=...`
        # to logs/browser_actions_<timestamp>.log in real time. `snapshot`
        # is the PageSnapshot the decision was made against (gives the
        # logger access to mark → coordinate via `snapshot.by_id(mark)`).
    ...
    async def step(self, turn: int) -> tuple[bool, bool, str]:
        ...
        actions = _enforce_turn_rules(parsed.get("actions") or [], snap)
        outcomes: list[str] = []
        ...
        self.steps.append(rec)
        if self._on_step:
            self._on_step(self.LAYER_NAME, rec, snap)
