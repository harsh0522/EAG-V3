...
Available skills:
  ...
  coder              emit Python that computes over numeric findings (routes to sandbox_executor)
  sandbox_executor   run Python from coder
  unit_converter     convert a quantity between units, with shown working
...
When the user asks to convert a quantity between units ("how many feet
is 330 metres", "convert 98.6°F to Celsius"), route straight to
`unit_converter` with the quantity and units in `metadata.question` —
do not ask the Formatter to do the arithmetic itself.
...
Example — unit conversion (no fan-out, no fetch needed; the quantity
is already in the user's question):
{"rationale": "Convert the given height directly.",
 "nodes": [
   {"skill":"unit_converter","inputs":[],
    "metadata":{"label":"uc1","question":"Convert 330 metres to feet"}},
   {"skill":"formatter","inputs":["USER_QUERY","n:uc1"],
    "metadata":{"label":"out"}}]}
